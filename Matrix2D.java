...
public class Matrix2D {
    ...
    /**
     * A basic constructor to allocate but not otherwise initialize a
     * matrix, so by the rules of Java, all entries are initially 0.
     * 
     ...
     * @param value The new value to be assigned at the input row and column
     *              location.
     */
    ...
    public boolean equals(Object other) {
        ...
        for (int row = 0; row < data.length; row++) {
            for (int col = 0; col < data.length; col++) {
                if (data[row][col] != o.data[row][col]) {
                    return false;
                }
            }
        }
